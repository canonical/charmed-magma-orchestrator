...
from charm import MagmaOrc8rCtracedCharm  # noqa: E402

logger = logging.getLogger(__name__)
METADATA = yaml.safe_load(Path("./metadata.yaml").read_text())

APPLICATION_NAME = SERVICE_NAME = "orc8r-ctraced"
...
class TestOrc8rCtracedUpgrade:
    ...
    async def test_workload_process_started_by_correct_command(
        ...
        assert MagmaOrc8rCtracedCharm.STARTUP_COMMAND in processes.values()
    ...
    async def test_workload_services_match_charm_config(self, ops_test, setup, build_and_refresh):
        expected_ports = [
            {"name": "grpc", "port": 9180, "protocol": "TCP", "targetPort": 9118},
            {"name": "grpc-internal", "port": 9190, "protocol": "TCP", "targetPort": 9218},
            {"name": "http", "port": 8080, "protocol": "TCP", "targetPort": 10118},
        ]
