...
from charm import MagmaOrc8rDeviceCharm  # noqa: E402

logger = logging.getLogger(__name__)
METADATA = yaml.safe_load(Path("./metadata.yaml").read_text())

APPLICATION_NAME = SERVICE_NAME = "orc8r-device"
...
class TestOrc8rDeviceUpgrade:
    ...
    async def test_workload_process_started_by_correct_command(
        ...
        assert MagmaOrc8rDeviceCharm.STARTUP_COMMAND in processes.values()
    ...
    async def test_workload_services_match_charm_config(self, ops_test, setup, build_and_refresh):
        expected_ports = [
            {"name": "grpc", "port": 9180, "protocol": "TCP", "targetPort": 9106},
            {"name": "grpc-internal", "port": 9190, "protocol": "TCP", "targetPort": 9306},
        ]
