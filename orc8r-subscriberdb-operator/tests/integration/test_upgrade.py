...
APPLICATION_NAME = SERVICE_NAME = "orc8r-subscriberdb"
CHARM_NAME = "magma-orc8r-subscriberdb"
CHARM_RESOURCES = {
...
class TestOrc8rSubscriberDBUpgrade:
    ...
    async def test_workload_process_started_by_correct_command(
        ...
        expected_startup_command = (
            "/var/opt/magma/bin/subscriberdb -run_echo_server=true -logtostderr=true -v=0"
        )
        ...
        assert expected_startup_command in processes.values()
    ...
    async def test_workload_services_match_charm_config(self, ops_test, setup, build_and_refresh):
        expected_ports = [
            {"name": "grpc", "port": 9180, "protocol": "TCP", "targetPort": 9083},
            {"name": "grpc-internal", "port": 9190, "protocol": "TCP", "targetPort": 9183},
            {"name": "http", "port": 8080, "protocol": "TCP", "targetPort": 10083},
        ]
