...
APPLICATION_NAME = SERVICE_NAME = "orc8r-smsd"
CHARM_NAME = "magma-orc8r-smsd"
CHARM_RESOURCES = {
...
class TestOrc8rSmsdUpgrade:
    ...
    async def test_workload_process_started_by_correct_command(
        ...
        expected_startup_command = (
            "/var/opt/magma/bin/smsd -logtostderr=true -run_echo_server=true -v=0"
        )
    ...
    async def test_workload_services_match_charm_config(self, ops_test, setup, build_and_refresh):
        expected_ports = [
            {"name": "grpc", "port": 9180, "protocol": "TCP", "targetPort": 9120},
            {"name": "grpc-internal", "port": 9190, "protocol": "TCP", "targetPort": 9220},
            {"name": "http", "port": 8080, "protocol": "TCP", "targetPort": 10086},
        ]
