...
APPLICATION_NAME = SERVICE_NAME = "orc8r-policydb"
CHARM_NAME = "magma-orc8r-policydb"
CHARM_RESOURCES = {
...
class TestOrc8rPolicydbUpgrade:
    ...
    async def test_workload_process_started_by_correct_command(
        ...
        expected_startup_command = (
            "/var/opt/magma/bin/policydb -run_echo_server=true -logtostderr=true -v=0"
        )
    ...
    async def test_workload_services_match_charm_config(self, ops_test, setup, build_and_refresh):
        expected_ports = [
            {"name": "grpc", "port": 9180, "protocol": "TCP", "targetPort": 9085},
            {"name": "grpc-internal", "port": 9190, "protocol": "TCP", "targetPort": 9185},
            {"name": "http", "port": 8080, "protocol": "TCP", "targetPort": 10085},
        ]
