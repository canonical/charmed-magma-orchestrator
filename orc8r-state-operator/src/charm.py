...
class MagmaOrc8rStateCharm(CharmBase):
    ...
    def __init__(self, *args):
        ...
        self._service_patcher = KubernetesServicePatch(
            charm=self,
            ports=[
                ServicePort(name="grpc", port=9180, targetPort=9105),
                ServicePort(name="grpc-internal", port=9190, targetPort=9305),
            ],
            additional_labels={"app.kubernetes.io/part-of": "orc8r-app"},
        )
        startup_command = "/usr/bin/envdir /var/opt/magma/envdir /var/opt/magma/bin/state -logtostderr=true -v=0"
