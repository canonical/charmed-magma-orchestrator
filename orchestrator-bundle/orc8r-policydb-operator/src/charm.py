...
class MagmaOrc8rPolicydbCharm(CharmBase):
    ...
    def __init__(self, *args):
        ...
        self.framework.observe(
            self.on.magma_orc8r_policydb_pebble_ready, self._on_magma_orc8r_policydb_pebble_ready
        )
        self.framework.observe(
            self._db.on.database_relation_joined, self._on_database_relation_joined
        )
        self._service_patcher = KubernetesServicePatch(
            self, [("grpc", 9180, 9085), ("http", 8080, 10085)], service_name="orc8r-policydb"
        )
    ...
    def _on_database_relation_joined(self, event):
        ...
        if self.unit.is_leader() and db_connection_string is not None:
            ...
            self._configure_magma_orc8r_policydb(db_connection_string)
        elif event.database != self.DB_NAME or db_connection_string is None:
            event.defer()
            return

    def _configure_magma_orc8r_policydb(self, db_conn_str):
