...
from charm import MagmaOrc8rStateCharm
...
class TestCharm(unittest.TestCase):
    ...
    def test_given_initial_status_when_get_pebble_plan_then_content_is_empty(self):
        initial_plan = self.harness.get_container_pebble_plan("magma-orc8r-state")
    ...
    def test_given_pod_is_leader_when_database_relation_joined_event_then_database_is_set_correctly(  # noqa: E501
        ...
        with patch.object(MagmaOrc8rStateCharm, "DB_NAME", self.TEST_DB_NAME):
    ...
    def test_given_ready_when_get_plan_then_plan_is_filled_with_magma_orc8r_state_service_content(
        ...
        with patch(
            ...
            self.harness.charm.on.magma_orc8r_state_pebble_ready.emit(event)
        expected_plan = {
            "services": {
                "magma-orc8r-state": {
                    "startup": "enabled",
                    "summary": "magma-orc8r-state",
                    "override": "replace",
        ...
        updated_plan = self.harness.get_container_pebble_plan("magma-orc8r-state").to_dict()
