...
class MagmaOrc8rTenantsCharm(CharmBase):
    ...
    def __init__(self, *args):
        ...
        self.framework.observe(
            self.on.magma_orc8r_tenants_pebble_ready, self._on_magma_orc8r_tenants_pebble_ready
        )
        self.framework.observe(
            self._db.on.database_relation_joined, self._on_database_relation_joined
        )
        self._service_patcher = KubernetesServicePatch(
            self, [("grpc", 9180, 9110), ("http", 8080, 10110)], service_name="orc8r-tenants"
        )
    ...
    def _on_magma_orc8r_tenants_pebble_ready(self, event):
        if not self._check_db_relation_has_been_established():
            ...
            event.defer()
            return
        self._configure_magma_orc8r_tenants()
    ...
    def _configure_magma_orc8r_tenants(self):
