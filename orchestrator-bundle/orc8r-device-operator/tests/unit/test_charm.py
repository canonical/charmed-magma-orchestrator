...
from charm import MagmaOrc8rDeviceCharm
...
class TestCharm(unittest.TestCase):
    ...
    def setUp(self):
        self.harness = Harness(MagmaOrc8rDeviceCharm)
    ...
    def test_given_initial_status_when_get_pebble_plan_then_content_is_empty(self):
        initial_plan = self.harness.get_container_pebble_plan("magma-orc8r-device")
    ...
    def test_given_pod_is_leader_when_database_relation_joined_event_then_database_is_set_correctly(  # noqa: E501
        ...
        with patch.object(MagmaOrc8rDeviceCharm, "DB_NAME", self.TEST_DB_NAME):
    ...
    @patch("charm.MagmaOrc8rDeviceCharm._namespace", new_callable=PropertyMock)
    @patch("charm.MagmaOrc8rDeviceCharm._check_db_relation_has_been_established")
    def test_given_ready_when_get_plan_then_plan_is_filled_with_magma_orc8r_device_service_content(
        ...
        with patch(
            "charm.MagmaOrc8rDeviceCharm._get_db_connection_string", new_callable=PropertyMock
        ) as get_db_connection_string:
            get_db_connection_string.return_value = self.TEST_DB_CONNECTION_STRING
            self.harness.charm.on.magma_orc8r_device_pebble_ready.emit(event)
        expected_plan = {
            "services": {
                "magma-orc8r-device": {
                    "startup": "enabled",
                    "summary": "magma-orc8r-device",
                    "override": "replace",
                    ...
                    "environment": {
                        ...
                        "SQL_DRIVER": "postgres",
                        "SQL_DIALECT": "psql",
                        "SERVICE_HOSTNAME": "magma-orc8r-device",
        ...
        updated_plan = self.harness.get_container_pebble_plan("magma-orc8r-device").to_dict()
