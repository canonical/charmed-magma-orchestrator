...
class MagmaOrc8rDeviceCharm(CharmBase):
    ...
    def __init__(self, *args):
        ...
        super().__init__(*args)
        self._container_name = self._service_name = "magma-orc8r-device"
        self._container = self.unit.get_container(self._container_name)
        self._db = pgsql.PostgreSQLClient(self, "db")  # type: ignore[attr-defined]
        self.framework.observe(
            self.on.magma_orc8r_device_pebble_ready, self._on_magma_orc8r_device_pebble_ready
        )
        self.framework.observe(
            self._db.on.database_relation_joined, self._on_database_relation_joined
        )
        self._service_patcher = KubernetesServicePatch(
            self, [("grpc", 9180, 9106)], service_name="orc8r-device"
        )

    def _on_magma_orc8r_device_pebble_ready(self, event):
        if not self._check_db_relation_has_been_established():
            ...
            event.defer()
            return
        self._configure_magma_orc8r_device()
    ...
    def _configure_magma_orc8r_device(self):
