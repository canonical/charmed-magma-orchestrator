...
class MagmaOrc8rSubscriberdbcacheCharm(CharmBase):
    DB_NAME = "magma_dev"
    ...
    def __init__(self, *args):
        ...
        self.framework.observe(
            self.on.magma_orc8r_subscriberdb_cache_pebble_ready,
            self._on_magma_orc8r_subscriberdb_cache_pebble_ready,
        )
        self.framework.observe(
            self._db.on.database_relation_joined, self._on_database_relation_joined
        )
        self._service_patcher = KubernetesServicePatch(
            self, [("grpc", 9180, 9105)], service_name="orc8r-subscriberdb-cache"
        )
    ...
    def _on_magma_orc8r_subscriberdb_cache_pebble_ready(self, event):
        if not self._check_db_relation_has_been_established():
            ...
            event.defer()
            return
        self._configure_magma_orc8r_subscriberdb_cache()
    ...
    def _configure_magma_orc8r_subscriberdb_cache(self):
...
if __name__ == "__main__":
    main(MagmaOrc8rSubscriberdbcacheCharm)
