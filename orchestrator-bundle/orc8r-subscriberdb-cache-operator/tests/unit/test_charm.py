...
from charm import MagmaOrc8rSubscriberdbcacheCharm
...
class TestCharm(unittest.TestCase):
    ...
    def setUp(self):
        self.harness = Harness(MagmaOrc8rSubscriberdbcacheCharm)
    ...
    def test_given_initial_status_when_get_pebble_plan_then_content_is_empty(self):
        initial_plan = self.harness.get_container_pebble_plan("magma-orc8r-subscriberdb-cache")
    ...
    def test_given_pod_is_leader_when_database_relation_joined_event_then_database_is_set_correctly(  # noqa: E501
        ...
        with patch.object(MagmaOrc8rSubscriberdbcacheCharm, "DB_NAME", self.TEST_DB_NAME):
    ...
    @patch("charm.MagmaOrc8rSubscriberdbcacheCharm._namespace", new_callable=PropertyMock)
    @patch("charm.MagmaOrc8rSubscriberdbcacheCharm._check_db_relation_has_been_established")
    def test_given_ready_when_get_plan_then_plan_is_filled_with_magma_orc8r_subscriberdb_cache_service_content(  # noqa: E501
        ...
        with patch(
            "charm.MagmaOrc8rSubscriberdbcacheCharm._get_db_connection_string",
            new_callable=PropertyMock,
        ) as get_db_connection_string:
            get_db_connection_string.return_value = self.TEST_DB_CONNECTION_STRING
            self.harness.charm.on.magma_orc8r_subscriberdb_cache_pebble_ready.emit(event)
        expected_plan = {
            "services": {
                "magma-orc8r-subscriberdb-cache": {
                    "startup": "enabled",
                    "summary": "magma-orc8r-subscriberdb-cache",
                    "override": "replace",
