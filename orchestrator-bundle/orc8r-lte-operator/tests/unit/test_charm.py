...
class TestCharm(unittest.TestCase):
    ...
    def test_given_initial_status_when_get_pebble_plan_then_content_is_empty(self):
        initial_plan = self.harness.get_container_pebble_plan("magma-orc8r-lte")
    ...
    def test_given_pod_is_leader_when_database_relation_joined_event_then_database_is_set_correctly(  # noqa: E501
        ...
        with patch.object(MagmaOrc8rLteCharm, "DB_NAME", self.TEST_DB_NAME):
    ...
    @patch("charm.MagmaOrc8rLteCharm._namespace", new_callable=PropertyMock)
    @patch("charm.MagmaOrc8rLteCharm._check_db_relation_has_been_established")
    def test_given_ready_when_get_plan_then_plan_is_filled_with_magma_orc8r_lte_service_content(
        ...
        with patch(
            "charm.MagmaOrc8rLteCharm._get_db_connection_string", new_callable=PropertyMock
        ) as get_db_connection_string:
            get_db_connection_string.return_value = self.TEST_DB_CONNECTION_STRING
            self.harness.charm.on.magma_orc8r_lte_pebble_ready.emit(event)
        expected_plan = {
            "services": {
                "magma-orc8r-lte": {
                    "startup": "enabled",
                    "summary": "magma-orc8r-lte",
                    "override": "replace",
        ...
        updated_plan = self.harness.get_container_pebble_plan("magma-orc8r-lte").to_dict()
